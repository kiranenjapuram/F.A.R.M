...
def run():
	rospy.Subscriber('motor_topic',Float64,callback)
	rospy.spin()
...
def callback(angle):
	...
	if angle.data > 0:                                                ##FOR CLOCKWISE DIRECTION
                rospy.loginfo("rotating motor %f clockwise",angle.data)
		...
		rospy.loginfo("rotating motor %f anticlockwise",angle.data)
		a_step_count=(angle.data)/1.8
		rospy.loginfo(float(a_step_count))
		#a_step_count=round(a_step_count)
		#a_step_count=float(a_step_count)
		#a_step_count=a_step_count*2
		#rospy.loginfo(float(a_step_count))
		for x in numpy.arange((a_step_count)):
			 #GPIO.output(EN, GPIO.LOW) 
